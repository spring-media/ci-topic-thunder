...
class NLPipe:
    ...
    * Similarity methods: normalize, cosine, kmeans, agglomorative, etc
    ...
    def load_finetuned_model(self, model_path,bucket_name):
        """ Load model from S3 Bucket """
        if not bucket_name:
            bucket_name = os.environ.get('S3_BUCKET')
        
        try:
            f = boto3.s3.download_file(bucket=bucket_name,key=model_path)

        except Exception as e:
            print('Could not download model from s3.',e)
    ...
    def cleantext(self, dataframe: pd.DataFrame, **kwargs):
        return [preprocess_articles_for_bert(dataframe, **kwargs)]
    ...
    @chain(device_in='cpu', together=True)
    def normalize(self, both):
        if not torch.is_tensor(
            both):
            both = torch.tensor(both)
